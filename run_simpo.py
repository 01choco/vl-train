...
class TrainingArguments(transformers.TrainingArguments):
    ...
    data_path: str = field(default="MMInstruction/VLFeedback")
    img_path: str = field(default="/images")
...
def make_vlfeedback_paired_dataset(local_rank, data_path, ig_path):
    ...
    def set_format(sample):
        ...
        img_path = f'{ig_path}/{sample["id"]}.jpg'
        sample["instruction"] = qwen_vl_prompt_format(prompt, [img_path])
        return sample
    ...
    def make_batch_pairs(sample):
        ...
        for sample_idx, comps in enumerate(sample["completions"]):
            prompt = sample["instruction"][sample_idx]

            for comp_idx1, comp_idx2 in combinations(range(len(comps)), 2):
                comp1, comp2 = comps[comp_idx1], comps[comp_idx2]
                anno1, anno2 = comp1["annotations"], comp2["annotations"]
                ...
                if avg_score1 > avg_score2:
                    chosen = comp1["response"]
                    rejected = comp2["response"]
                elif avg_score2 > avg_score1:
                    chosen = comp2["response"]
                    rejected = comp1["response"]
                else:
                    continue
        ...
        print(">>> batch pair output lengths:",
        len(converted_sample["prompt"]),
        len(converted_sample["chosen"]),
        len(converted_sample["rejected"]))
...
def train():
    ...
    # Load data
    dataset = make_vlfeedback_paired_dataset(training_args.local_rank, training_args.data_path, training_args.img_path)
